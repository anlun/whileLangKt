...
write(r)
    ...
    @Test fun test15() {
        val exprText = "1 + 2 * 8"
        Assert.assertEquals("(1 + (2 *\n      8))", exprParser.get(exprText)?.print(10))
    }

    @Test fun test16() {
        val exprText = "1 + 2 * 8"
        Assert.assertEquals("(1 + (2 * 8))", exprParser.get(exprText)?.print(14))
    }

    @Test fun test17() {
        val ast = stmtParser.get(logpowProgram)
        val ppProgram = ast?.print(100)
                ?: throw Exception("Failed to pretty print program.")
        val newAst = stmtParser.get(ppProgram)
                ?: throw Exception("Failed to parse pretty-printed program.")
        Assert.assertEquals(listOf(625),newAst.interpret(listOf(5, 4)))
    }
}